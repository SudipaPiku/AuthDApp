...
export default function AddProduct() {
    ...
    const schema = yup.object({
        name: yup.string().required('Required!').max(250, 'Product Name Should be less than 250 characters').test('no Num', "Number not allowed", async (val) => { if (val) { return await !val.match(/[0-9]+/) } return false }).test('noSpecial', "Special characters not allowed", async (val) => { if (val) { return await val.match(/[a-z]/i) } return false }),
    ...
    async function addProduct(values) {
        try {
            ...
            const Response = await provider.sendTransaction('addProduct', [values.productId, hash, values.price, values.name]);

           if(Response === 'no')
           {
            const Response1 = await provider.callTransaction('productDetails', [productId]);

            setPopup(Response1);

            setPopup('You are not the owner of this smart contract !!!');
            history.push('/addproductEr');
           }else{
            ...
            setPopup('Failed to add product. Product id is used before !!!!');
            console.log(error)
        }
    }
